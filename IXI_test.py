...
from voxelmorph.py.utils import jacobian_matrix as JM
...
if __name__ == '__main__':
    ...
    for line in tqdm(test_lines, desc="Processing samples", ncols=100):
        ...
        # 获取moving文件的标签文件名
        moving_label_file = 'dataset/IXI_data_nii/Test/label/subject_{}_label.nii.gz'.format(moving_file_name)
        fixed_label_file = 'dataset/IXI_data_nii/Test/label/subject_{}_label.nii.gz'.format(fixed_file_name)

        # 配准后的label文件名
        moved_label_file = os.path.join(f'output/{dataset_name}/moved_label',
                                        fixed_file_name + '_' + moving_file_name + '_moved_label.nii.gz')

        # 配准后的文件名
        moved_file_name = fixed_file_name + '_' + moving_file_name + '_moved.nii.gz'
        moved_file = os.path.join(f'output/{dataset_name}/moved', moved_file_name)

        # 保存形变场
        warp_file_name = fixed_file_name + '_' + moving_file_name + '_warp.nii.gz'
        warp_file = os.path.join(f'output/{dataset_name}/warp', warp_file_name)

        # 保存变化后的变形场
        warp_cor_file_name = fixed_file_name + '_' + moving_file_name + '_warp_cor.nii.gz'
        warp_cor_file = os.path.join(f'output/{dataset_name}/warp_cor', warp_cor_file_name)

        # 模型文件名
        model_file = 'models/IXI/2000.pt'
