...
def get_jacobi_matrix(displacement):
    '''
...
def resize_nifti_files_in_directory(img, new_shape):
    """
    Resize all .nii.gz files in the specified directory and its subdirectories to the new shape.

    Args:
        directory_path (str): The path to the directory containing the .nii.gz files.
        new_shape (tuple): The new target shape as a tuple (z, y, x).
    """
    # 遍历目录及其子目录下的所有.nii.gz文件

    # 获取图像的尺寸
    img_shape = img.shape

    # 这里应该用img.get_fdata()获取图像数据
    data = img.get_fdata()
    # 获取输入图像的数据类型
    ...
    # 如果你的新尺寸与原始尺寸不成比例，需要调整缩放比例以保持纵横比
    x_scale = new_x / x
    y_scale = new_y / y
    z_scale = new_z / z

    # 然后使用新的缩放比例来执行重采样
    resized_data = scipy.ndimage.zoom(data, zoom=(z_scale, y_scale, x_scale), order=1)
    # 将重采样后的数据类型回到输入图像的数据类型
    resized_data = resized_data.astype(data_type)
    # 将重采样后的数据类型回到输入图像的数据类型
    resized_data = resized_data.astype(data_type)
    ...
    print('original shape: {} \t resampled shape:{}'.format(img_shape, resized_img.shape))
    return resized_img
...
def jacobian_matrix(disp):
    ...
    if len(volshape) == 3:
        J = np.stack([J[0], J[1], J[2]], axis=-1)
    else:  # 2D
        J = np.stack([J[0], J[1]], axis=-1)
