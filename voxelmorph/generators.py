...
def volgen_ct_ct(vol_names,batch_size=1,segs=None,np_var='vol',pad_shape=None,resize_factor=1,add_feat_axis=True):
    ...
    while True:
        ...
        imgs1 = [py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
        vols1 = [np.concatenate(imgs1, axis=0)]

        imgs2 = [py.utils.load_volfile(vol_names[i].replace('_0000','_0001'), **load_params) for i in indices]
        ...
        if segs is True:
            ...
            load_params['np_var'] = 'seg'  # be sure to load seg
            s = [py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
            vols1.append(np.concatenate(s, axis=0))
        elif isinstance(segs, list):
            # assume segs is a corresponding list of files or preloaded volumes
            s = [py.utils.load_volfile(segs[i], **load_params) for i in indices]
            vols1.append(np.concatenate(s, axis=0))
...
def ct_to_ct(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    zeros = None
    gen = volgen_ct_ct(vol_names, batch_size=batch_size, **kwargs)
    while True:
        t = next(gen)
        scan1, scan2 = t[0][0], t[1][0]

        ormalized_scan1 = (scan1 - np.min(scan1)) / (np.max(scan1) - np.min(scan1))
        normalized_scan2 = (scan2 - np.min(scan2)) / (np.max(scan2) - np.min(scan2))

        # some induced chance of making source and target equal
        if prob_same > 0 and np.random.rand() < prob_same:
            if np.random.rand() > 0.5:
                ormalized_scan1 = normalized_scan2
            else:
                normalized_scan2 = ormalized_scan1

        # cache zeros
        if not no_warp and zeros is None:
            shape = ormalized_scan1.shape[1:-1]
            zeros = np.zeros((batch_size, *shape, len(shape)))

        invols = [ormalized_scan1, normalized_scan2]
        outvols = [normalized_scan2, ormalized_scan1] if bidir else [normalized_scan2]
        if not no_warp:
            outvols.append(zeros)
